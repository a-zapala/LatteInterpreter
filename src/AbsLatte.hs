...
data ArgType a = RefArgType a (Type a) | ValueArgType a (Type a)
  deriving (Eq, Ord, Show, Read)

instance Functor ArgType where
    fmap f x = case x of
        RefArgType a type_ -> RefArgType (f a) (fmap f type_)
        ValueArgType a type_ -> ValueArgType (f a) (fmap f type_)
data Stmt a
...
instance Functor Type where
    fmap f x = case x of
        ...
        Bool a -> Bool (f a)
        Void a -> Void (f a)
        Fun a argtypes type_ -> Fun (f a) (map (fmap f) argtypes) (fmap f type_)
data Expr a
    = EVar a Ident
    | ELitInt a Integer
    | ELitTrue a
    ...
    | EApp a Ident [Expr a]
    | EString a String
    | Anonymoys a (Type a) [Arg a] (Block a)
    | Neg a (Expr a)
    | Not a (Expr a)
...
instance Functor Expr where
    fmap f x = case x of
        ...
        Anonymoys a type_ args block -> Anonymoys (f a) (fmap f type_) (map (fmap f) args) (fmap f block)
